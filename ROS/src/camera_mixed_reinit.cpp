...
void Homography_call(Mat img_scene, ros::Time stamp)
{
	...
	Ptr<Feature2D> fd=cv::xfeatures2d::SURF::create(1000); // Threshold=1000
	  ...
	  {    
		...
		tras=-Rot*R_acum*ts_decomp[SOL]*Position.at<double>(2);
		R_acum=R_acum*Rs_decomp[SOL].t();
		angles=fast_Euler_angles(Rot*R_acum*Rot);

		// Si se mantienen pequeñas variaciones durante 15 periodos, se reinician a 0 los giros en X e Y
		if((angles.roll-Prev_angles.roll < 0.01 && angles.roll-Prev_angles.roll > -0.01) && (angles.pitch-Prev_angles.pitch < 0.01 && angles.pitch-Prev_angles.pitch > -0.01))
			cont++; 
		else
			cont=0;
		...
		if(cont==15)
		{
			angles.roll=0;
			angles.pitch=0;
			R_acum=Rot*ComposeRotation(angles)*Rot;
