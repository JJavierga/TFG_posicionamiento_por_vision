...
EulerAngles Euler_curr={0,0,0}; //Ángulos leidos de IMU
...
Mat Ref_Position=Mat::zeros(3,1,CV_64FC1);
EulerAngles Ref_angles={0,0,0};
	...
	Ptr<Feature2D> fd=cv::xfeatures2d::SURF::create(1000); // Threshold=1000
	  ...
	  {    
		...
		for (int i = 0; i < solutions; i++)
			...
			{
				Var_cost=FrameVar(Rs_decomp[i], Prev_rot_matrix); //Calcula diferencia entre el vector de giro y la matriz de giro
				if(Var_cost<Min_var)
				{
					Min_var=Var_cost;
					SOL=i;
		...
		tras=-Rot*R_acum*ts_decomp[SOL]*Ref_Position.at<double>(2); 
		angles=fast_Euler_angles(Rot*R_acum*Rs_decomp[SOL].t()*Rot);
        	Prev_rot_matrix=Rs_decomp[SOL];

		// Si el movimiento ha cambiado mucho con respecto a la referencia
		if(abs(tras.at<double>(0))>Ref_Position.at<double>(2)*0.217 || abs(tras.at<double>(1))>Ref_Position.at<double>(2)*0.386 || abs(tras.at<double>(2))>1 || sqrt(tras.dot(tras))>Ref_Position.at<double>(2)*0.4 || abs(angles.roll-Ref_angles.roll)>0.1 || abs(angles.pitch-Ref_angles.pitch)>0.1)
	  	{
			// Calcula posición
	  		Position.at<double>(0)=Ref_Position.at<double>(0)+tras.at<double>(0);	
	  		Position.at<double>(1)=Ref_Position.at<double>(1)+tras.at<double>(1);	
	  		Position.at<double>(2)=Ref_Position.at<double>(2)+tras.at<double>(2);	
			
			// Si pasa durante 10 periodos seguidos
			cont++;
			if(cont==10)
			{
			// Cambia referencia
			img_scene.copyTo(img_object);
 	 		Ref_Position.at<double>(0)=Position.at<double>(0);
 	 		Ref_Position.at<double>(1)=Position.at<double>(1);
 	 		Ref_Position.at<double>(2)=Position.at<double>(2);
			Ref_angles=angles;
			R_acum=R_acum*Rs_decomp[SOL].t();
			std::cout << "Cambio referencia: " << stamp << std::endl;
			cont=0;
			}
	  	}
		else
...
void Aruco_call(Mat img_scene, ros::Time stamp)
{
	...
	if (flag23==1)
	{
		...
		// Almacena el resultado en el archivo
	     file.open("/home/javier/pruebas/MasPruebas/Data/Mixed.txt", std::ios_base::app);
      	     file << stamp << ", " <<  Angles.roll << "," << Angles.pitch << "," << Angles.yaw << "," << Position.at<double>(0)-0.15 << "," <<  Position.at<double>(1) << "," << Position.at<double>(2) << std::endl;
	     file.close();
	
		// Si ha superado los 7m, termina el despegue y tomamos la imagen actual como referencia
	     if(Position.at<double>(2)>7)
	     {
		flag_h7=1;
		Ref_Position.at<double>(0)=Position.at<double>(0)-0.15;
		Ref_Position.at<double>(1)=Position.at<double>(1);
		Ref_Position.at<double>(2)=Position.at<double>(2);
	     }
	}	
