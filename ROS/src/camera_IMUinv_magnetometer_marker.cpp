...
void imuCallback(const sensor_msgs::Imu::ConstPtr& msg)
{
	...
	Euler_curr=Quat2Euler(msg->orientation); // Leer orientación del magnetómetro
...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
	 ...
	 EulerAngles Angles=Euler_curr;
	  ...
	  // Matriz de rotación con los ángulos del magnetómetro
	...
	if (flag23==1)
	  {
		...
		// Cambia el resultado a cámara con respecto al marcador según magnetómetro
	     Mat t_aux=(Mat_<double>(3,1) << tvecs[i][0], tvecs[i][1], tvecs[i][2]);
	     Mat tras= -1*rot_mat_imu*Rot*t_aux;
